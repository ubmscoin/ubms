...
- AI나 샤딩 기술은 현재 적용되지 않았으나, 미래 확장 가능성 존재
...
- **소·중규모(100~10,000 노드):**  
  UBMS 방식은 릴레이·투표자 소수 구조와 blueprint 설정으로 합의 메시지를 극도로 줄여, 이더리움 PoS 대비 합의 속도와 트래픽 효율이 탁월하다.
- **대규모(100,000 노드 이상):**  
  UBMS도 하트빗 트래픽이 10만 건 이상 발생하여 CF 문제에 직면할 수 있으나, 합의 자체는 여전히 제한된 소수에 의해 빠르게 처리된다. 이더리움 PoS도 위원회가 기하급수적으로 증가하므로 샤딩·L2 등 별도 기술이 필수.  
  UBMS 또한 AI·샤딩 등 추가 솔루션을 도입해 확장성을 확보할 수 있다.

**결과적으로**, UBMS는 blueprint로 릴레이와 투표자 규모를 미리 지정해 **합의 트래픽을 효율적으로 통제**한다는 큰 장점을 갖는다. 다만, 네트워크가 초대형일 경우 하트빗(가십)으로 인한 CF가 문제가 되며, 이 부분에서 이더리움도 유사한 한계를 겪는다.  
궁극적으로, 대규모 확장에는 샤딩이나 AI 최적화가 필요하며, 그 전까지는 UBMS가 중소 규모에서 압도적 효율성을 발휘한다.

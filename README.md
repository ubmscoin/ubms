...
- **PoS 소각 메커니즘 상세**  
  ...
  - **스테이커가 1,000~9,999명**이면  
    - **POS 할당량 (스테이커 수 / 10,000) × 100%만 분배, 나머지 소각**


$$\text{POS 분배율 공식} = \max\left(0.1,~\min\left(1,~\frac{\text{스테이커 수}}{10,000}\right)\right)\times 100\%$$
$$\text{POS 소각률 공식} = \left[1-\max\left(0.1,~\min\left(1,~\frac{\text{스테이커 수}}{10,000}\right)\right)\right]\times 100\%$$



- **분배되는 POS 할당량**은 Shifted Sigmoid 가중치로 각 참여자에게 배분됩니다.
- **소각되는 분량**은 누구도 가져갈 수 없습니다.
...
- **참여자 적을 때 공급 자동 제한**  
  → 인플레이션 방지, 코인 가치 안정화
- **스테이킹 활성화 유도**  
  → 더 많은 참여자 모으는 구조적 인센티브 제공
- **장기적 생태계 균형 확보**  
  → PoS 독식 방지 + 공급량 통제 + 네트워크 건전성 유지

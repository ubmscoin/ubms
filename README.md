...
POB 행위증명 채굴은 사용자가 코인을 전송할 때 발생하는 수수료(fee)를,  레퍼런스에 기여한 사람들에게 분배하여 보상하는 방식입니다.  
여기서 **디스트리뷰터**란 UBMS 코인을 다른 사람에게 추천하거나 홍보한 사용자의 지갑 주소를 의미하며, 이 주소가 반드시 입력되어야만 UBMS 코인이 올바르게 작동합니다.
...
즉, UBMS 네트워크 내에서는 코인 전송 시 발생하는 수수료가 단순히 네트워크 운영비용으로 사용되는 것이 아니라, 사용자 간의 상호 보상 메커니즘을 통해 자연스럽게 홍보와 참여를 유도하는 효과를 기대할 수 있습니다.

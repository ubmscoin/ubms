...
- AI나 샤딩 기술은 현재 적용되지 않았으나, 노드가 10,000을 넘게 되면 장차 확장 가능합니다.
...
UBMS 가십 프로토콜은 TTL이 5로 기본값 설정되 있으며, 한 번의 전파에서 최대 5개의 논리적으로 가까운 피어(lookup 결과)를 사용합니다.  
...
UBMS 방식에서는 한 번에 5개의 lookup 결과를 활용하여 가십 메시지를 전파하고, TTL 5 내에서 빠른 동기화를 이루는 것이 목표입니다.  
기본값으로 가십전파는 이론상 3125개의 노드에 도달할수 있습니다. 노드가 늘면 TTL 값을 늘려 전파 범위를 늘리면 됩니다.

...
- AI나 샤딩 기술은 현재 적용되지 않았으나, 노드가 10000을 넘게되면 장차 확장 가능합니다.
...
1) **릴레이(R) 및 투표자(V) 선정 (청사진, 즉 blueprint 적용)**  
...
**결과적으로**, UBMS는 blueprint로 릴레이와 투표자 규모를 미리 지정해 **합의 트래픽을 효율적으로 통제**한다는 큰 장점을 갖습니다. 다만, 네트워크가 초대형일 경우 하트빗으로 인한 CF가 문제가 되며, 이 부분에서 이더리움도 유사한 한계를 겪는고있습니다.  
...
- **PSAN(Role-Polymorphic Self-Adjusting Network) 역할다형성 자가조정 네트워크:**
  네트워크 혼잡을 해결한 UMBS의 해법 상기의 항목 **4.네트워크 구조**에 상세히 설명함   
...
|------------|--------------------------------------------------------------------|----------------------------------------------------------------------------------|--------------------------------------------------------------------------------|
| **속도**   | 단순한 버퍼 조작으로 최소 오버헤드, 매우 빠름                        | 재귀적 처리와 길이 계산 등으로 오버헤드가 있어 상대적으로 느림                     | 템플릿 기반 최적화로 직접 메모리 복사하여 최고 빠름                              |
| **메모리** | 간단한 버퍼 연산으로 메모리 사용이 매우 효율적                        | 동적 메모리 할당 및 재귀 호출로 추가 메모리 사용 발생                              | 고정 사이즈 데이터 및 템플릿 처리로 불필요한 메모리 사용 최소화                   |
| **코드**   | 커스텀 구현이지만 구조가 단순하여 중간 정도의 복잡성                  | 다양한 데이터 타입과 중첩 구조 지원으로 복잡하고 구현 난이도가 높음                  | S_MESSAGE 공용체로 모든 유형의 구조체를 전달할 수 있어 간단함                     |
| **확장성** | 고정된 데이터 구조로 확장이 제한적                                   | 다양한 데이터 구조 지원은 가능하나, 재귀적 설계로 확장 시 구현 복잡도가 증가          | 템플릿 특수화로 새로운 데이터 타입 추가가 용이하여 확장이 뛰어남                   |
| **디버깅** | 단순한 구조로 인해 디버깅 및 유지보수가 상대적으로 수월함              | 복잡한 재귀 구조로 인해 에러 발생 시 추적 및 수정이 어려울 수 있음                   | 표준 C++ 코드와 명확한 템플릿 구조로 일관된 코드사용으로 유지보수가 용이함            |
| **유연성** | 특정 데이터 구조에 최적화되어 있어 유연성은 낮은 편                   | 다양한 데이터 타입과 중첩 지원으로 유연하지만, 그에 따른 성능 저하가 단점             | 모든유형의 데이터 전달가능하며 성능이 뛰어나다                                    |

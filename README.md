...
UBMS의 채굴 인센티브은 채굴자의 기여도와 참여자의 지분에 따라 공정하게 처리됩니다.
...
- **대지분 독식 방지**  
  인센티브이 선형이 아닌 곡선화되어 고지분자가 급격히 독식하지 못함
...
POB 행위증명 채굴은 사용자가 코인을 전송할 때 발생하는 수수료(fee)를, 레퍼런스에 기여한 사람들에게 인센티브를 제공하는 온체인 방식의 채굴입니다.  
행위증명의 인센티브은 블럭에 기록되고 UBMS 시스템이 수행하기에 누락이 없고 행위의 인센티브을 장담합니다.  
...
즉, 전체 수수료 금액을 \( F \)라 할 때, 각 디스트리뷰터에게 지급되는 인센티브은 다음 식으로 계산됩니다.

$$
\begin{aligned}
\text{1대 인센티브} &= F \times 0.60, \\
\text{2대 인센티브} &= F \times 0.20, \\
\text{3대 인센티브} &= F \times 0.10, \\
\text{4대 인센티브} &= F \times 0.07, \\
\text{5대 인센티브} &= F \times 0.03.
\end{aligned}
$$
...
이와 같이 인센티브은 각 단계별로 고정된 비율에 따라 나누어 지급되며, 디스트리뷰터들의 인센티브이 이루어집니다.

## 디스트리뷰터 관리 및 필수 입력

- UBMS 코인의 정상 작동을 위해서는 반드시 거래 시 **디스트리뷰터**의 지갑 주소가 입력되어야 합니다.  
- 거래를 전송하는 사용자가 자신을 홍보한 사람의 주소 목록을 입력할 경우, 이 목록은 역순으로 정렬되어 가장 최근에 홍보한 사람이 1대(60%)에 해당하며, 그 이후 순서대로 2대, 3대, 4대, 5대의 인센티브 비율이 적용됩니다.

이와 같은 구조는 사용자들이 서로를 홍보하고, 그에 따른 수수료 인센티브을 통해 네트워크의 활성화 및 확산에 기여하도록 설계되었습니다.  

...
| 비교 항목                  | RGB 프로토콜                   | 카르다노(eUTXO)               | UBMS(UTXO-Based Metadata Smart Contract) |
|----------------------------|--------------------------------|--------------------------------|------------------------------------------|
| 확장성                     | 비트코인 네트워크에서 제한적 확장 | 병렬 처리로 높은 확장성        | 페이로드 기반으로 유연한 확장 가능          |
| 스마트 컨트랙트 표현 방식  | UTXO에 토큰 상태만 저장        | UTXO 자체에 스마트 컨트랙트 검증 포함 | UTXO의 페이로드에서 스마트 컨트랙트 수행 |
| 스마트 컨트랙트 실행 방식  | 온체인 실행 불가능              | 트랜잭션 검증 시 실행 (온체인)  | 온체인/오프체인 혼합 방식               |
| 데이터 저장 위치           | 오프체인에서 관리               | 블록체인 내부에 저장           | UTXO 내부 페이로드로 저장          |
| 조건 기반 실행 가능 여부  | 지원하지 않음                   | 특정 조건 만족 시 실행 가능    | 페이로드에서 조건 설정 후 실행 가능    |
| 수명주기(TTL) 설정  | 지원하지 않음                  | 기본적으로 지원하지 않음  | TTL 필드에 직접 포함 가능            |
| 변경 가능성                | 토큰 발행 후 변경 불가          | 한 번 생성된 eUTXO는 변경 불가 | 데이터 일부 수정 가능 |
| 오프체인 데이터 저장  | 대부분 오프체인에서 처리        | 온체인에서 모든 검증 수행      | 일부 데이터를 오프체인에서도 관리 가능  |
| 블록체인 과부하 문제       | 별도의 오프체인 검증 필요       | 온체인 스마트 컨트랙트 실행으로 과부하 발생 가능 | 페이로드 최적화로 과부하 방지 가능 |
| 개발자 친화성              | 비트코인 스크립트 기반으로 복잡  | Plutus/Haskell 사용 (진입장벽 높음) |  기반으로 쉽게 사용 가능          |
| 보안성                     | 비트코인 보안 모델 유지         | 블록체인 내부 검증으로 높은 보안 | 블록체인+오프체인 조합 시 보안 모델 필요 |

---
# 장차 UBMS기술로 구현될 토큰, DeFi, NFT, DAO, 자동화된 계약 개념설명 

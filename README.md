...
| 조건 기반 여부  | 지원하지 않음                   | 특정 조건 만족 시 실행     | 페이로드에서 조건 설정 후 실행   |
| 수명주기(TTL) 설정  | 지원하지 않음                  | 기본적으로 지원하지 않음  | TTL 필드에 직접 포함             |
...
| 개발자 친화성              | 비트코인 스크립트 기반으로 복잡  | Plutus/Haskell 사용 (진입장벽 높음) |  기반으로 쉽게 사용          |

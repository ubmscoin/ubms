...
UBMS의 POS 인센티브 분배 시스템에는  
**참여자가 적을 때 자동으로 일부 인센티브를 소각(Burn)하는 공급 조절 메커니즘**이 내장되어 있습니다.

#### **소각 공식 및 구조**

- 전체 블록 인센티브의  
  - **5%는 PoW 채굴자에게 무조건 지급**  
  - **95%는 PoS 스테이커를 위한 할당량**(이 할당량을 100%로 간주)  
- **PoS 소각 메커니즘 상세**  
  - **스테이커가 100,000명 이상**이면  
    - **POS 할당량 100%를 시그모이드 가중치로 전액 분배** (0% 소각)
  - **스테이커가 10,000명 이하**이면  
    - **POS 할당량 10%만 시그모이드로 분배, 90% 소각**
  - **스테이커가 10,000~99,999명**이면  
    - **POS 할당량 (스테이커 수 / 100,000) × 100%만 분배, 나머지 소각**


$$\text{POS 분배율 공식} = \max\left(0.01,~\min\left(1,~\frac{\text{스테이커 수}}{100,000}\right)\right)\times 100\%$$
$$\text{POS 소각률 공식} = \left[1-\max\left(0.01,~\min\left(1,~\frac{\text{스테이커 수}}{100,000}\right)\right)\right]\times 100\%$$



- **분배되는 POS 할당량**은 Shifted Sigmoid 가중치로 각 참여자에게 배분됩니다.
- **소각되는 분량**은 완전히 소멸되어 누구도 가져갈 수 없습니다.

#### **실제 동작 예시 (POS 할당량 기준)**

| 스테이커 수     | 분배 비율 | 소각 비율 | 비고                    |
|---------------|-----------|----------|-------------------------|
| 0             | 0%        | 100%     | PoS 보상 전액 소각      |
| 8000           | 10%       | 90%      | 10%만 분배, 90% 소각    |
| 50,000         | 50%       | 50%      | 50%만 분배, 50% 소각    |
| 90,500         | 95%       | 5%       | 95%만 분배, 5% 소각     |
| 100,000 이상   | 100%      | 0%       | 전액 분배, 소각 없음    |
...
UBMS의 POS 인센티브 시스템은  
- **총 인센티브의 5%는 채굴자에게**,  
- **95%는 Sigmoid 가중치를 통해 비선형 형태가**되며, 참여자가 일정수 이하일때는 소각메커니즘이 작동합니다.  

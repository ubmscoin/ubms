...
기준 액면가로 1ubms당 시장가를 **10,000원**으로 설정하고, 이에 해당하는 기저 수수료는 **1,000원**으로 정합니다.  
...
아래 표는 시장가 $$v$$를 1,000원에서 100,000,000원까지 10배 단위로 증가시킬 때 계산된 최종 수수료 $$F$$를 보여줍니다 (혼잡팩터 $$CF=1.0$$로 가정).
...
이 계산 방식는 기준 외부 데이터를 10,000원으로 통일하여 경제팩터를 산출하고, 네트워크 혼잡팩터와 함께 최종 수수료를 산출함으로써,  
